...
import { COS_BUCKET, COS_REGION, isCosConfigured, loadCosConfig, saveCosConfig, syncManifest, uploadContentFile } from "./cosAssets";
...
export default function AdminPanel({ content, onContentChange }) {
  ...
  const [aboutPrimary, setAboutPrimary] = useState(() => content.profile?.aboutPrimary || "我叫孙海峰，是一名专注内容叙事与视觉表达的 AI 短剧剪辑师。熟悉真人短剧的粗剪、精剪与节奏把控，也能独立完成 AI 漫剧从小说改写、剧本分镜、资产图建立、视频生成到剪辑成片的完整流程。");
  ...
  const [displayName, setDisplayName] = useState(() => content.profile?.displayName || "孙海峰");
  const [heroRole, setHeroRole] = useState(() => content.profile?.heroRole || "剪辑师 / AI设计师 / AI漫剧");
  const [footerCopyright, setFooterCopyright] = useState(() => content.profile?.footerCopyright || "© 2024—2026 孙海峰 · 保留所有权利");
  ...
  useEffect(() => {
    ...
    setAboutPrimary(profile.aboutPrimary || "我叫孙海峰，是一名专注内容叙事与视觉表达的 AI 短剧剪辑师。熟悉真人短剧的粗剪、精剪与节奏把控，也能独立完成 AI 漫剧从小说改写、剧本分镜、资产图建立、视频生成到剪辑成片的完整流程。");
    ...
    setDisplayName(profile.displayName || "孙海峰");
    setHeroRole(profile.heroRole || "剪辑师 / AI设计师 / AI漫剧");
    setFooterCopyright(profile.footerCopyright || "© 2024—2026 孙海峰 · 保留所有权利");
  }, [content.profile]);
  ...
  return <aside className="admin-drawer" aria-label="内容管理"><header><strong>内容管理</strong><button type="button" onClick={() => setOpen(false)}><X size={20} /></button></header><div className="admin-scroll">
    <p className="admin-note">桶：{COS_BUCKET}（{COS_REGION === "ap-guangzhou" ? "广州" : COS_REGION}）。视频、图片和内容清单都会自动同步到 COS，访客刷新后即可看到。</p>
